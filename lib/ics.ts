...
export function importableCalendarEntries(events: ImportedCalendarEvent[]) {
  ...
  return events
    .filter((event) => isAfter(event.start, addDays(now, -1)) && isBefore(event.start, windowEnd))
    .map((event): CalendarEntry => {
      const type = inferType(event.title, event.description);
      const durationHours = event.end ? Math.max(0.5, (event.end.getTime() - event.start.getTime()) / (1000 * 60 * 60)) : 1;
      return {
        day: toDayName(event.start),
        title: event.title,
        type,
        load: inferLoad(event.title, type, durationHours),
        time: format(event.start, "MMM d, h:mma"),
